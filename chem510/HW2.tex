...
\solution
    An optical aberration is a deviation in the behavior of optical elements from the ideal. Two types are:
    \begin{itemize}
        \item chromatic: when broadband light is passed through an optical component, the different wavelengths follow different paths. This results in a spreading of the colors
        \item monochromatic: aberration in the lens that affects monochromatic light. These result from non-ideally shaped lenses and include coma and astigmatism
    \end{itemize}
...
\solution
    \part
        \begin{align*}
            m \lambda &= d \left( \sin{\theta} + \sin{\phi} \right) \\
            \phi &= \arcsin{\left( \frac{m \lambda}{d} - \sin{\theta} \right)} \\
            &= \arcsin{
                       \left(
                           \frac{
                               \SI{240}{\nano\meter}
                            }{
                                \SI{3600}{grooves\per\milli\meter}^{-1}
                            } 
                            - \sin{\ang{30}}
                       \right) 
            } \\
    ...
    \part
        ...
        \end{align*}
    \part
        To a point, longer wavelengths will be diffracted to larger angles in the first mode.
    \part
        \begin{align*}
            \phi_2 &= \phi_1 \\
            \arcsin{\left( \frac{m_2 \lambda_2}{d} - \sin{\theta} \right)} &= \arcsin{\left( \frac{m_1 \lambda_1}{d} - \sin{\theta} \right)} \\
            { 2 \times \lambda_2 } &= { 1 \times \lambda_1 } \\
            \lambda_2 &= \frac{1}{2} \SI{350}{\nano\meter} \\
            &= \SI{175}{\nano\meter}
        \end{align*}
    \part
        \begin{align*}
            \Delta \lambda &= \frac{\lambda}{m} \\
            \lambda + \Delta \lambda &= \SI{600}{\nano\meter} \\
            \lambda &= \frac{1}{2} \SI{600}{\nano\meter} = \SI{300}{\nano\meter} \\
            \Delta \lambda &= \SI{300}{\nano\meter}
        \end{align*}
...
\solution
    \part
        \begin{align*}
            D_l &= f \times D_a \\
            R_d^{-1} &= f \times D_a \\
            D_a &= \frac{1}{R_d \times f} \\
            &= \frac{1}{\SI{320}{\milli\meter} \times \SI{1.5}{\nano\meter\per\milli\meter}} \\
            &= \SI{0.0021}{\radian\per\nano\meter} \\
            &= \SI{0.12}{\degree\per\nano\meter}
        \end{align*}
    \part
        \begin{align*}
            \left( \SI{1.5}{\nano\meter\per\milli\meter} \right)^{-1} \\
            \SI{0.67}{\milli\meter\per\nano\meter}
        \end{align*}
    \part
        \begin{align*}
            S_g &= R_d W \\
